...
public class AgentsNormal extends AppCompatActivity {
    ...
    TextView name, hostname, process_id, external_ip, internal_ip, listener, process_name, username, lastseen_time, os_details, checkin_time, delay;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.hide();
        setContentView(R.layout.activity_agents_normal);
                ...
                if(position > 0){
                    ...
                    try {
                        ...
                        for (int i = 0; i < getValues.length(); i++) {
                            ...
                            //for each option for that module, dynamically draw and editText and set the hint as that options name
        ...
        get_results.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                //Create Linear layout and add scrolling TextView to it.
                LinearLayout mainlayout = new LinearLayout(MyApplication.getContext());
                mainlayout.setBackgroundColor(Color.parseColor("#212121"));
                mainlayout.setLayoutParams(new LinearLayout.LayoutParams(600, 800));

                TextView resultsView = new TextView(MyApplication.getContext());
                resultsView.setTextColor(Color.parseColor("#00FF00"));
                resultsView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT));
                resultsView.setPadding(20,20,20,20);
                resultsView.setMovementMethod(new ScrollingMovementMethod());
                mainlayout.addView(resultsView);
                ...
                try{
                    ...
                    resultsView.setText("[*] AGENT RESULTS: \n");
                    resultsView.append(agent_results);
                }catch (Exception e){
                    e.printStackTrace();
                }
        ...
        events_logged.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext(), android.R.style.Theme_DeviceDefault_Dialog_NoActionBar_MinWidth);
                builder.setView(mainlayout);
                builder.setMessage("Logged Events");
                AlertDialog alert = builder.create();
                alert.setCanceledOnTouchOutside(true);
                alert.show();
            }
        ...
        get_creds.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                try{
                    ...
                    resultsView.setText("[*] CREDENTIALS STORED! \n");
                    //loop over array to get all objects
                    for (int i = 0; i < getValues.length(); i++) {
                        jObj1 = getValues.getJSONObject(i);
                        String creds = jObj1.toString().replace(",", "\n").replace("{", "").replace("}", "").replace("\\\\r\\\\n", "\n").replace('"', ' ').replace("[", "").replace("]", "").replace("\\", "");
                        resultsView.append(creds);
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext(), android.R.style.Theme_DeviceDefault_Dialog_NoActionBar_MinWidth);
                builder.setView(mainlayout);
                builder.setMessage("Stored Credentials");
                AlertDialog alert = builder.create();
                alert.setCanceledOnTouchOutside(true);
                alert.show();
            }
